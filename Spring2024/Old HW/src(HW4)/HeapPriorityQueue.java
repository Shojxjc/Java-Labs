...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void upheap2(int j) {
    while (j > 0) {            // continue until reaching root (or break statement)
      int p = parent(j);
      if (compare(heap.get(p), heap.get(j)) >= 0) break; // heap property verified
      swap(j, p);
      j = p;                                // continue from the parent's location
    }
  }
  ...
  public Entry<K,V> insert2(K key, V value) throws IllegalArgumentException {
    checkKey(key);      // auxiliary key-checking method (could throw exception)
    Entry<K,V> newest = new PQEntry<>(key, value);
    heap.add(newest);                      // add to the end of the list
    upheap2(heap.size() - 1);               // upheap newly added entry
    return newest;
  }
